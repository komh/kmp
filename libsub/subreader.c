...
static subtitle *sub_read_line_sami(FILE *fd, subtitle *current) {
...
static subtitle *sub_read_line_microdvd(FILE *fd,subtitle *current) {
...
static subtitle *sub_read_line_mpl2(FILE *fd,subtitle *current) {
    char line[LINE_LEN+1];
    char line2[LINE_LEN+1];
    char *p, *next;
    int i;
...
static subtitle *sub_read_line_subrip(FILE *fd, subtitle *current) {
    char line[LINE_LEN+1];
...
static subtitle *sub_read_line_subviewer(FILE *fd,subtitle *current) {
    char line[LINE_LEN+1];
    int a1,a2,a3,a4,b1,b2,b3,b4;
    char *p=NULL;
    int i,len;
...
static subtitle *sub_read_line_subviewer2(FILE *fd,subtitle *current) {
    char line[LINE_LEN+1];
    int a1,a2,a3,a4;
    char *p=NULL;
    int i,len;
...
static subtitle *sub_read_line_vplayer(FILE *fd,subtitle *current) {
    char line[LINE_LEN+1];
    int a1,a2,a3;
...
static subtitle *sub_read_line_rt(FILE *fd,subtitle *current) {
...
static subtitle *sub_read_line_ssa(FILE *fd,subtitle *current) {
/*
 ...
 */
static subtitle *sub_read_line_pjs(FILE *fd,subtitle *current) {
    char line[LINE_LEN+1];
...
static subtitle *sub_read_line_mpsub(FILE *fd, subtitle *current) {
    char line[LINE_LEN+1];
    float a,b;
    int num=0;
    char *p, *q;
...
static subtitle *sub_read_line_aqt(FILE *fd,subtitle *current) {
    char line[LINE_LEN+1];
    char *next;
    int i;
...
static subtitle *sub_read_line_subrip09(FILE *fd,subtitle *current) {
    char line[LINE_LEN+1];
    int a1,a2,a3;
    char * next=NULL;
    int i,len;
...
static subtitle *sub_read_line_jacosub(FILE * fd, subtitle * current)
{
...
static int sub_autodetect (FILE *fd, int *uses_time) {
    char line[LINE_LEN+1];
    int i,j=0;
    char p;
...
void    subcp_open (char *current_sub_cp)
{
    ...
    if (current_sub_cp){
        if ((icdsc = iconv_open (tocp, current_sub_cp)) != (iconv_t)(-1)){
            dprintf("SUB: opened iconv descriptor.\n");
            sub_utf8 = 2;
        } else
            dprintf("SUB: error opening iconv descriptor.\n");
    }
}
...
void    subcp_close (void)
{
    if (icdsc != (iconv_t)(-1)){
        ...
        icdsc = (iconv_t)(-1);
        dprintf("SUB: closed iconv descriptor.\n");
    }
}
...
subtitle* subcp_recode (subtitle *sub)
{
    ...
    while (l){
        ...
        if (!(ot = (char *)malloc(op - icbuffer + 1))){
            dprintf("SUB: error allocating mem.\n");
            l++;
            break;
        }
        *op='\0' ;
...
subtitle* subcp_recode1 (subtitle *sub)
{
  ...
  while (l){
     ...
     if (iconv(icdsc, &ip, &ileft,
          &op, &oleft) == (size_t)(-1)) {
    dprintf("SUB: error recoding line (2).\n");
    return sub;
     }
     *op='\0' ;
  }
...
void subcp_recode2 (subtitle *sub)
{
    char subt_buf[512];
    int j;
    extern char cpfrom[10];
    extern char cpto[10];

    for(j=0;j<sub->lines;j++)
    {
        memset(subt_buf,0,sizeof(subt_buf));
        cp2cp(cpfrom, cpto,
              sub->text[j], subt_buf, strlen(sub->text[j]));
        strcpy(sub->text[j], subt_buf);
    }
}
#endif
...
subtitle* sub_fribidi (subtitle *sub, int sub_utf8)
{
  ...
  while (l) {
    ...
    if(len > LINE_LEN) {
      dprintf("SUB: sub->text is longer than LINE_LEN.\n");
      l++;
      break;
    }
...
void* guess_cp(FILE *fd, char *preferred_language, char *fallback)
{
    ...
    dprintf("ENCA supported languages: ");
    for (i = 0; i < langcnt; i++) {
    dprintf("%s ", languages[i]);
    }
    dprintf("\n");
